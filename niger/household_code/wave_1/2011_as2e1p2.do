...
* open log
	cap 	log 	close
	log 	using	"`logout'/2011_as2e1p2_1", append
...
* drop observations in which it was not harvest season. Identify harvest season if farmers say they worked the land this rainy season
	tab as02eq04
	drop if as02eq04 == 2 & cropid == .
	*** dropped 108 observations

* drop if household has not completed harvest	
	drop if as02eq07a == 0 | as02eq07a == 999999
	*** 3166 observations dropped
...
* replace miscoded variables as missing 
	...
	*** 0 changes (obs = 999998 and 999999) - seems to be . in many cases for Niger
...
* examine quantity harvested variable
	...
	*** average  172.725, max 15000, min 0 

* replace any +3 s.d. away from median as missing
	replace			harvkg = . if harvkg > `r(p50)'+(3*`r(sd)')
	sum				harvkg, detail
	*** replaced 84 values, max is now 1610 
...
* how did the imputation go?
	...
	drop			harvkg_1_
	*** imputed 89 out of 8801 total observations
	*** mean from 135 to 141, max at 2208, (no change in min or max)
...
* merge in regional information 
	...
	*** 8800 matched, 1 not matched from master, 1824 not matched from using
...
* check to see if we have prices for all crops
	...
	*** no prices for fonio, wheat (ble), gourd, souchet
	
* drop if we are missing prices
	drop			if p_crop == .
	*** dropped 26 observations
...
* replace any +3 s.d. away from median as missing, by cropid
	...
	*** mean 23, max 436
	...
	sum				cropvalue, detail
	*** replaced 280 values
	*** increases mean to 19, max to 115 
...
* how did the imputation go?
	...
	drop			cropvalue_1_
	*** imputed 89 out of 8774 total observations
	*** mean from 19.1 to 19.6, max at 116 (no change in max)
...
* replace any +3 s.d. away from median as missing
	...
	sum				mz_hrv
	*** replaced 123 values, max is now 780 instead of 1600
...
* how did the imputation go?
	...
	drop			mz_hrv_1_
	*** imputed 123 values out of 4024 total values
	*** mean from 163 to 165, max 780 (no change), min 0 (no change)
...
* create unique household-plot-crop identifier
	...
	keep 			clusterid hh_num ord field parcel region dept canton cropplot_id plot_id /// 
...
* save file
	customsave , idvar(plot_id) filename("2011_ase1p2_1.dta") ///
		path("`export'") dofile(2011_ase1p2_1) user($user)
