...
* open log
	cap log close
	log using "`logout'/wv2_PPSEC3", append


* **********************************************************************
* 1 - preparing ESS 2013/14 (Wave 2) - Post Planting Section 3 
* **********************************************************************

* load pp section 3 data
	use 		"`root'/sect3_pp_w2.dta", clear
...
* create conversion key 
	...
	*** 11,687 obs not matched from master data
	*** why is this...
	
	tab			local_unit _merge
	*** conversion facotrs only given for timad, boy, senga, and kert
	*** but some obs in timad, boy, senga, and kert are missing
	*** but measurements are also given in tilm, and other
...
* GPS on the larger side vs self-report
	tabulate 	gps if gps>2, plot	// GPS doesn't seem outrageous, 19 obs > 10 ha
...
* GPS on the smaller side vs self-report 
	summarize 	gps if gps<0.002 // ~2,500 obs
...
* evaluating correlations between various measures
	...
	*** 0.9372 - correlation outside this range very high!
	
	pwcorr 		gps rap if inrange(gps,0.002,4) 
	*** 0.6912 - higher than the overall range
	
	pwcorr 		selfreport_ha rap 	
	*** 0.0226 correlation - extremely low
	
	pwcorr 		selfreport_ha gps	
	*** 0.0189 correlation - very very low
	
	pwcorr 		selfreport_ha gps if inrange(gps,0.002,4) ///
					& inrange(selfreport_ha,0.002,4)
	*** 0.7981 - much much higher when range is restricted for both	
...
* make plotsize using GPS area if it is within reasonable range
	...
	*** 547 obs w/ selfreport_ha missing plotsize
...
* replace weeks worked equal to zero if missing
	...
	replace		pp_s3q27_z = 0 if pp_s3q27_z == . 
	
* find average # of days worked by first worker reported (most obs)
	sum 		pp_s3q27_c pp_s3q27_g pp_s3q27_k pp_s3q27_o pp_s3q27_s ///
					pp_s3q27_w pp_s3q27_ca
	*** pp_s3q27_c has by far the most obs, mean is 2.65
	
* replace days per week worked equal to 2.59 if missing and weeks were worked 
	replace		pp_s3q27_c = 2.65 if pp_s3q27_c == . &  pp_s3q27_b != 0 
	replace		pp_s3q27_g = 2.43 if pp_s3q27_g == . &  pp_s3q27_f != 0  
	replace		pp_s3q27_k = 2.47 if pp_s3q27_k == . &  pp_s3q27_j != 0  
	replace		pp_s3q27_o = 2.28 if pp_s3q27_o == . &  pp_s3q27_n != 0  
	replace		pp_s3q27_s = 2.13 if pp_s3q27_s == . &  pp_s3q27_r != 0  
	replace		pp_s3q27_w = 2.3 if pp_s3q27_w == . &  pp_s3q27_v != 0  
	replace		pp_s3q27_ca = 3 if pp_s3q27_ca == . &  pp_s3q27_z != 0  
...
* generating individual household labor rates
	...
	generate	laborhh_7 = pp_s3q27_z * pp_s3q27_ca
...
* look at fertilizer use
	generate	fert_any = 1 if pp_s3q15 == 1 | pp_s3q18 ==1 | pp_s3q20a == 1
...
* variable in the same observation	
	generate 	fert_u = pp_s3q16_a // urea
	replace		fert_u = 0 if pp_s3q16_a == . & pp_s3q19_a != .
	
	generate 	fert_d = pp_s3q19_a // DAP
	replace		fert_d = 0 if pp_s3q19_a == . & pp_s3q16_a != .
	
	generate 	kilo_fert = fert_u + fert_d
	label var 	kilo_fert "Kilograms of fertilizer applied (Urea and DAP only)"
	drop 		fert_u fert_d
	*** kilo_fert only captures Urea, DAP - 5,361 obs
...
* summarize fertilizer
	...
	*** median 0, mean 10.9, max 8,000

* replace any +3 s.d. away from median as missing
	replace			kilo_fert = . if kilo_fert > `r(p50)'+(3*`r(sd)')
	*** replaced 30 values, mean is now 6.93, max is now 334
