...
loc export = "C:\Users/$user\Dropbox\Weather_Project\results\MW_temp"

*start generating data to create graphs
use "`root'/appended_lp_tp.dta", clear
...
foreach l of local levels {
	...
	graph export "`export'/cdf_temp_`l'.png", as(png) replace
}
...
foreach l of local levels {
	...
	graph export "`export'/den_temp_`l'.png", as(png) replace
}
...
graph bar (mean) p90 p95 p99, over(varname, label(angle(45) labsize(vsmall))) yscale(r(0 1)) ///
	ylab(0 .2 .4 .6 .8 1, labsize(small)) ytitle("Share of Significant Point Estimates") title("p-Values by Weather Metric (k=360; n=6,116)") ///
	legend(pos(12) col(3) label(1 "p>0.90") label(2 "p>0.95") label(3 "p>0.99"))
	graph export "`export'/pval_varname_tp.png", as(png) replace

graph bar (mean) p90 p95 p99, over(sat, label(angle(45) labsize(vsmall))) yscale(r(0 1)) ///
	ylab(0 .2 .4 .6 .8 1, labsize(small)) ytitle("Share of Significant Point Estimates") title("p-Values by Satellite (k=960; n=6,116)") ///
	legend(pos(12) col(3) label(1 "p>0.90") label(2 "p>0.95") label(3 "p>0.99"))
	graph export "`export'/pval_sat_tp.png", as(png) replace

graph bar (mean) p90 p95 p99, over(ext, label(angle(45) labsize(vsmall))) yscale(r(0 1)) ///
	ylab(0 .2 .4 .6 .8 1, labsize(small)) ytitle("Share of Significant Point Estimates") title("p-Values by Extraction Method (k=288; n=6,116)") ///
	legend(pos(12) col(3) label(1 "p>0.90") label(2 "p>0.95") label(3 "p>0.99"))
	graph export "`export'/pval_ext_tp.png", as(png) replace

graph bar (mean) p90 p95 p99, over(depvar, label(angle(45) labsize(vsmall))) yscale(r(0 1)) ///
	ylab(0 .2 .4 .6 .8 1, labsize(small)) ytitle("Share of Significant Point Estimates") title("p-Values by Dependant Variable (k=1,440; n=6,116)") ///
	legend(pos(12) col(3) label(1 "p>0.90") label(2 "p>0.95") label(3 "p>0.99"))
	graph export "`export'/pval_depvar_tp.png", as(png) replace

graph bar (mean) p90 p95 p99, over(regname, label(angle(45) labsize(vsmall))) yscale(r(0 1)) ///
	ylab(0 .2 .4 .6 .8 1, labsize(small)) ytitle("Share of Significant Point Estimates") title("p-Values by Specification (k=480; n=6,116)") ///
	legend(pos(12) col(3) label(1 "p>0.90") label(2 "p>0.95") label(3 "p>0.99"))
	graph export "`export'/pval_regname_tp.png", as(png) replace
