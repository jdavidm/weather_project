...
foreach folder of local folderList {
	...
	foreach file in `fileList' {
		...
		weather rf_ , rain_data ini_month(1) fin_month(8) day_month(1) keep(y3_hhid) save("`root'/`folder'/`dat'_`ext'_`sat'.dta")
...
foreach folder of local folderList {
	...
	foreach file in `fileList' {
		...
		weather tmp_ , temperature_data growbase_low(10) growbase_high(30) ini_month(1) fin_month(8) day_month(1) keep(y3_hhid) save("`root'/`folder'/`dat'_`ext'_`sat'.dta")
		}
}
