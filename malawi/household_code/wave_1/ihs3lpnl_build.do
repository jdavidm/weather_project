...
* does
	* merges weather data into IHS3 long panel
...
* open log	
	log 	using 		"`logout'/ihs3lpnl_build", append
...
* import the .dta houeshold file
	use 		"`rooth'/hhfinal_ihs3lpnl.dta", clear
...
* save file
	qui: compress
	customsave 	, idvar(case_id) filename("lp1_merged.dta") ///
		path("`export'") dofile(ihs3lpnl_build) user($user)
