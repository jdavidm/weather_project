...
* does
	* merges IHS3 short panel data with weather data
...
* open log	
	log 	using 		"`logout'/ihs3spnl_merge", append
...
* define local with all files in each sub-folder	
	...
	* loop through each file in the above local
		...
		* merge weather data with household data
			merge 1:1 	case_id using "`rooth'/hhfinal_ihs3spnl.dta"
		...
		* save file
		customsave 	, idvar(case_id) filename("sp1_`ext'_`sat'_merged.dta") ///
...
* define local with all files in each sub-folder	
	...
	* loop through each file in the above local
		...
		* save file
		customsave 	, idvar(case_id) filename("sp1_`ext'_tp`sat'_merged.dta") ///
		path("`export'") dofile(ihs3spnl_merge) user($user)
		}
}
