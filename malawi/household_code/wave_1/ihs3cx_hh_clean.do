...
* does
	* cleans WB data set for IHS3 cross section
...
* define paths
	...
	loc		root 	= 	"G:/My Drive/weather_project/household_data/malawi/wave_1/raw"
	loc		export 	= 	"G:/My Drive/weather_project/household_data/malawi/wave_1/refined"
...
* merge cross section household files with ag season files
	use 		"`source'/AG_MOD_A_FILT.dta", clear

	merge 1:1	case_id	using	"`root'/ihs3cx_hh.dta"
...
* drop and keep variables
	...
	keep 		case_id region district urban ta ea_id strata cluster ///
				hhweightR1 intmonth intyear ag_c0a ag_j0a qx_type ///
...
* generate year variables and season variables
	gen 		year = 2008 if ag_c0a == 1
	replace 	year = 2009 if ag_c0a == 2
...
* save data
	customsave	, idvar(case_id) filename(hhfinal_ihs3cx.dta) ///
				path("`export'") dofile(ihs3cx_hh_clean) user($user)
