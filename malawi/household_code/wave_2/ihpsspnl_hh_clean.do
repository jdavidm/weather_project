...
* does
	* cleans WB data set for IHPS short panel
...
* define paths
	loc		root 	= 	"G:/My Drive/weather_project/household_data/malawi/wave_2/raw"
	loc		export 	= 	"G:/My Drive/weather_project/household_data/malawi/wave_2/refined"
	loc		logout 	= 	"G:/My Drive/weather_project/household_data/malawi/logs"

* open log	
	log 	using 		"`logout'/ihpsspnl_hh_clean", append
...
* load data
	use 		"`root'/ihpsspnl_hh.dta", clear 

* keep only the variables we need
	keep 		y2_hhid case_id region district urbanR2 ta ea_id urbanR1 /// 
				strataR1 strataR2 cluster hhweightR1 hhweightR2 intmonth ///
				intyear qx_type splitoffR2 tracking_R1_to_R2 distance_R1_to_R2 ///
				rs_harvest_value* rsmz_harvest* ds_harvest_value* dsmz_harvest* ///
...
* save data
	customsave	, idvar(y2_hhid) filename(hhfinal_ihpsspnl.dta) ///
				path("`export'") dofile(ihpsspnl_hh_clean) user($user)
