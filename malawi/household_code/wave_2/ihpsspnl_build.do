...
* does
	* merges weather data into IHPS3 short panel
...
* open log	
	log 	using 		"`logout'/ihpsspnl_build", append
...
* import the .dta houeshold file
	use 		"`rooth'/hhfinal_ihpsspnl.dta", clear
...
* save file
	qui: compress
	customsave 	, idvar(y2_hhid) filename("sp2_merged.dta") ///
		path("`export'") dofile(ihpsspnl_build) user($user)
