...
* does
	* merges IHPS long panel data with weather data
...
* define paths
	loc		rootw 	= 	"$data/weather_data/malawi/wave_2/refined"
	loc		rooth 	= 	"$data/household_data/malawi/wave_2/refined"
	loc		export 	= 	"$data/merged_data/malawi/wave_2"
	loc		logout 	= 	"$data/merged_data/malawi/logs"

* open log
	log 	using 		"`logout'/ihpslpnl_merge", append
...
* define local with all files in each sub-folder
	...
	* loop through each file in the above local
		...
		* merge weather data with household data
			merge 1:1 	y2_hhid using "`rooth'/hhfinal_ihpslpnl.dta"
		...
		* save file
		customsave 	, idvar(y2_hhid) filename("lp2_`ext'_`sat'_merged.dta") ///
		path("`export'") dofile(ihpslpnl_merge) user($user)
		}
}
...
* define local with all files in each sub-folder
	...
	* loop through each file in the above local
		...
		* save file
		customsave 	, idvar(y2_hhid) filename("lp2_`ext'_tp`sat'_merged.dta") ///
		path("`export'") dofile(ihpslpnl_merge) user($user)
		}
}
