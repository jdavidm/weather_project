...
*temp regressions
local fileList : dir "`root'" files "cx*tp*.dta" 
...
mean_season- z_gdd data year hhid satellite extraction
...
tempname myresults
postfile `myresults' str4 depvar str5 regname str25 varname betatemp serain adjustedr loglike dfr using myresults.dta, replace
...
*-temperature and fe			
foreach v of varlist `temp' {
	qui: xtreg lnrs_harvest_valuehaimp `v' i.year, fe
...
*-temperature and squared temperature and inputs
foreach v of varlist `temp' {
...
clear all
*/
save "`results'/appended_cx_tp.dta", emptyok replace

loc fileList : dir "`export'" files "myresults_cx*tp*" 
foreach file in `fileList'{
...
save "`results'/appended_cx_tp.dta", replace
