...
* merge in plot size data
	...
	*** 0 out of 3,356 missing in master 
...
* impute yield outliers
	...
	*** reduces mean from 2,431 to 1,005
...
* impute value per hectare outliers 
	...
	tabstat			vl_yld vl_yldimp, ///
						f(%9.0f) s(n me min p1 p50 p95 p99 max) c(s) longstub
	*** reduces mean from 1,030 to 326
...
* impute labor outliers, right side only 
	...
	*** note that this code is slighlty different we don't use !inlist
	tabstat 		fert_ha fert_haimp, ///
						f(%9.0f) s(n me min p1 p50 p95 p99 max) c(s) longstub
	*** reduces mean from 1,969 to 137
...
* make labor days based on imputed labor days per hectare
	...
	*** 2,914 changes made (out of 3,356 obs!)
	*** what is happening here?? 2008 AG_SEC3A looks all right...
...
* verify values are accurate
	sum				tf_* cp_*
	*** the fertilizer values are off the charts compared to other waves
	*** that compared with the issue from line 248 mkae me suspicious
	*** what is happening?
