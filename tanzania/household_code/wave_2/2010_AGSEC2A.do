...
* inverstingating the high and low end of gps measurments
	* high end
	 ...
	 *** 483 obs > 2.4
		list		plotsize_gps plotsize_self if plotsize_gps>2.4 & !missing(plotsize_gps), sep(0)
		pwcorr		plotsize_gps plotsize_self if plotsize_gps>2.4 & !missing(plotsize_gps)
	 *** corr = 0.6864 (not terrible)
		sum 		plotsize_gps if plotsize_gps>3.958
	 *** 236 obs > 3.958
		list		plotsize_gps plotsize_self if plotsize_gps>3.95 & !missing(plotsize_gps), sep(0)
		pwcorr		plotsize_gps plotsize_self if plotsize_gps>3.95 & !missing(plotsize_gps)
...
*-->	replace 	plotsize_gps = . if plotsize_gps >= 20	
	...
	* low end
	 ...
	 *** 460 obs < 0.089
		list		plotsize_gps plotsize_self if plotsize_gps<0.09 & !missing(plotsize_gps), sep(0)
		pwcorr		plotsize_gps plotsize_self if plotsize_gps<0.09 & !missing(plotsize_gps)
	 *** corr = 0.0394 (very very low correlation)
		sum 		plotsize_gps if plotsize_gps<0.053
	 *** 238 obs < 0.05
		list		plotsize_gps plotsize_self if plotsize_gps<0.053 & !missing(plotsize_gps), sep(0)
		pwcorr		plotsize_gps plotsize_self if plotsize_gps<0.053 & !missing(plotsize_gps)
	 ...
	 *** 27 obs w/ plotsize_gps < 0.01 (including four zero values)
		...
		pwcorr		plotsize_gps plotsize_self if plotsize_gps<0.01 & !missing(plotsize_gps)
	 *** these all take values of 0, 0.0040469, or 0.0080937 (meaning pre-conversion values of 0, 0.01, or 0.02)
...
summarize 
sort plot_id
customsave , idvar(plot_id) filename(AG_SEC2A.dta) path("`export'") dofile(2010_AGSEC2A) user($user)
