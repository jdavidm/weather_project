...
* inverstingating the high and low end of gps measurments
	* high end
	 ...
	 *** 541 obs > 2.65
		list		plotsize_gps plotsize_self if plotsize_gps>2.65 & !missing(plotsize_gps), sep(0)
		pwcorr		plotsize_gps plotsize_self if plotsize_gps>2.65 & !missing(plotsize_gps)
	 *** corr = 0.8087 (very good)
		sum 		plotsize_gps if plotsize_gps>4.3
	 *** 270 obs > 4.3
		list		plotsize_gps plotsize_self if plotsize_gps>4.3 & !missing(plotsize_gps), sep(0)
		pwcorr		plotsize_gps plotsize_self if plotsize_gps>4.3 & !missing(plotsize_gps)
	...
	* low end
	 ...
	 *** 544 obs < 0.081
		list		plotsize_gps plotsize_self if plotsize_gps<0.081 & !missing(plotsize_gps), sep(0)
		pwcorr		plotsize_gps plotsize_self if plotsize_gps<0.081 & !missing(plotsize_gps)
	 *** corr = 0.0794 (pretty poor)
		sum 		plotsize_gps if plotsize_gps<0.049
	 *** 290 obs < 0.049
		list		plotsize_gps plotsize_self if plotsize_gps<0.049 & !missing(plotsize_gps), sep(0)
		pwcorr		plotsize_gps plotsize_self if plotsize_gps<0.049 & !missing(plotsize_gps)
