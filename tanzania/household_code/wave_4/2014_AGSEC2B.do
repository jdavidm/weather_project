 ...
 *** only 4 not missing, out of 5,537
 ...
 *** 3,337 missing both variables, these must be dropped
	drop 		if plotsize_self == . & plotsize_gps == .
	pwcorr 		plotsize_gps plotsize_self
 *** very strong corr (0.9985)
* its difficult to split four observations into a high end and a low end
		tab			plotsize_gps
		sum			plotsize_gps, detail
		sort		plotsize_gps
		list		plotsize_gps plotsize_self if !missing(plotsize_gps), sep(0)
 *** kind of a big discrepancy in the 4th ob but I'm not going to get rid of it
* dropping any '0' values, to be imputed later
		replace 	plotsize_gps = . if plotsize_gps == 0
 *** 0 changes made
* I will not drop any values at this time
...
* impute missing + irregular plot sizes using predictive mean matching
	...
	mi impute 	pmm plotsize_gps plotsize_self i.uq_dist, add(1) rseed(245780) noisily dots force knn(4) bootstrap
 *** had to change that knn value to 4 (from 5)
 *** error message: knn() may not exceed the size of the observed sample, from which imputed values are drawn
	mi 			unset
