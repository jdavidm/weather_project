...
* define paths
	loc		root 	=	"$data/weather_data/tanzania/wave_1/raw"
	loc		export	=	"$data/weather_data/tanzania/wave_1/daily"
...
foreach folder of loc folderList {
	...
	* save file
	customsave , idvar(hhid) filename("`dat'_`ext'_`sat'_daily.dta") ///
		path("`export'/`folder'") dofile(TZA_NPSY1_converter) user($user)
	}
}
