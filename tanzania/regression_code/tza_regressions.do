...
* TO DO:
	* everything!
	* just copied and pasted in mwi_regressions.does
	* need to re-write everything for TZA
...
* define paths
	loc		source	= 	"$data/regression_data/tanzania"
	loc		results = 	"$data/results_data/tanzania"
	loc		logout 	= 	"$data/regression_data/logs"

* open log	
	cap 	log		close
	log 	using 	"`logout'/tza_regressions", append
...
* read in data file
	use			"`source'/tza_complete.dta", clear
	...
	* rename ids to match Malawi
		rename		cx_id case_id
		rename		pl_id lpid
...
* save complete results
	customsave 	, idvarname(reg_id) filename("tza_complete_results.dta") ///
		path("`results''") dofile(tza_regression) user($user)
